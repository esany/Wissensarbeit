...
- Idempotency: stable record IDs and the persisted idempotency key `sha256:WA-HARVEST-SEED-003:P1:v1`; replay regression passes and duplicate IDs fail closed.
- Assurance: 73 tests pass; `python3 tools/work.py validate`, `audit`, and `derive` pass; the pre-existing Foundation harvest remains valid.
